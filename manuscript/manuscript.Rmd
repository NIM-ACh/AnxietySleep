...
data[j = .(edad = cent_disp(edad), .N)]
data[j = .(.N, `%` = round(.N/nrow(data)*100, 1)), by = sexo]
data[j = .(.N, `%` = round(.N/nrow(data)*100, 1)), by = region]
```
...
# Sexo
data[, cent_disp(beck_global), sexo]; autest(data, "sexo", "beck_global", markdown = T)$full |> cat()
data[, cent_disp(pits_global), sexo]; autest(data, "sexo", "pits_global", markdown = T)$full |> cat()

# Región
data[, cent_disp(beck_global), region]; autest(data, "region", "beck_global", markdown = T)$full |> cat()
data[, cent_disp(pits_global), region]; autest(data, "region", "pits_global", markdown = T)$full |> cat()

# Grupos etarios
data[, cent_disp(beck_global), cat_edad]; autest(data, "cat_edad", "beck_global", markdown = T)$full |> cat()
pairs_two_sample(data, "cat_edad", "beck_global")[j = lablr(.SD, T)$full, by = .(group1, group2)]

data[, cent_disp(pits_global), cat_edad]; autest(data, "cat_edad", "pits_global", markdown = T)$full |> cat()

data[, .N, by = n_personas_convive] 

data[, cent_disp(beck_global), by = n_personas_convive] 
autest(data, x = "n_personas_convive", y = "beck_global", paired = FALSE)
pairs_two_sample(data, x = "n_personas_convive", y = "beck_global")[p.value <= 0.05]

autest(data, x = "n_personas_convive", y = "pits_global")
...
# beck_global
lm(sqrt(beck_global) ~ sexo + region, data) |> anova() |> report::report_statistics()

# pits_global
lm(sqrt(pits_global) ~ sexo + region, data) |> anova() |> report::report_statistics()

data[-599,.SD, .SDcols = \(i) is.numeric(i) & all(!is.na(i))][-44] |> 
  cluster::pam(3) |> 
  factoextra::fviz_cluster(
    data = data[-599,.SD, .SDcols = \(i) is.numeric(i) & all(!is.na(i))][-44],
  )
```
...
model <- copy(data)[, `:=`(
  ...
  BAI = beck_global,
  PSQI = pits_global
)] |> lm(formula = sqrt(PSQI) ~ Sexo + sqrt(BAI))

model |> equatiomatic::extract_eq(use_coefs = TRUE)
...
Al analizar el efecto de $\sqrt{BAI}$ sobre $\sqrt{PSQI}$ mediante la implementación de un modelo de regresión lineal múltiple, hallamos que el incremento en 1 unidad en $\sqrt{BAI}$, se asoció a un aumento en 0.24 unidades en $\sqrt{PSQI}$ tras controlar por la influencia del sexo (CI~95%~[0.21, 0.27], *t*(614) = 15.94, *p* < 0.001;), lo que se expresa de mejor manera en la [Ecuación 1](#eq1). Este modelo explica una proporción estadísticamente significativa y sustancial de la varianza (R^2^ = 0.31, *F*(2, 614) = 140.1, *p* < 0.001, R^2^ ajustado = 0.31).

$$\widehat{\sqrt{PSQI}} = 2.14 + 0.13(Sexo_{Mujer}) + 0.24(\sqrt{BAI})$$
